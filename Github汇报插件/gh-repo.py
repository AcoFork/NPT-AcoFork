...
GROUP_IDS = [123456789]  # 你的QQ群号列表
CHECK_INTERVAL = 5  # 检查间隔时间（秒）
GITHUB_TOKEN = "ghp_XXXXXXXXXXXXXXXXXXXXXXXXXXXX"  # 你的GitHub个人访问令牌
...
async def check_github_repo(url):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            # 检查文件变动
            tree_url = f"{url}/git/trees/main?recursive=1"
            ...
            if repo_name not in latest_trees:
                ...
                if modified_files:
                    await report_file_changes(client, repo_name, modified_files)
            ...
            if repo_name not in latest_branches:
                ...
                for branch, commit_sha in current_branches.items():
                    if branch in previous_branches and commit_sha != previous_branches[branch]:
                        await check_branch_merge(client, repo_name, branch, previous_branches[branch], commit_sha)
...
async def report_file_changes(client, repo_name, modified_files):
    commit_summary, author = await get_latest_commit_info(client, repo_name)
...
async def check_branch_merge(client, repo_name, branch, old_commit, new_commit):
    ...
    if compare_data['merged']:
        base = compare_data['base_commit']['sha'][:7]
        head = compare_data['head_commit']['sha'][:7]
        msg = f"仓库 {repo_name} 的分支 {branch} 发生合并：\n{base} <- {head}"
        await send_group_message(msg)

async def get_latest_commit_info(client, repo_name):
    ...
    try:
        ...
        if commits_data:
            ...
            return commit_message, author
        return "无法获取提交信息", "未知作者"
    except Exception as e:
        logger.error(f"获取提交信息时出错：{e}")
        return "获取提交信息时出错", "未知作者"
